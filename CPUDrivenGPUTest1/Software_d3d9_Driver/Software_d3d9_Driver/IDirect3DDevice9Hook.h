...
public:
	...
	void DeviceLoadVertexDataBuffers(const unsigned vertCount, const std::vector<SHORT>* const posXbuffer, const std::vector<SHORT>* const posYbuffer, const std::vector<DWORD>* const invZbuffer, 
	...
	void DeviceLoadIndexBuffer(const unsigned short* const indices, const unsigned numIndices);

	void DeviceLoadVertexDataProcessedVerticesPosition(const D3DPRIMITIVETYPE PrimitiveType, const UINT primCount);
	void DeviceLoadVertexDataProcessedVerticesAttributes(const D3DPRIMITIVETYPE PrimitiveType, const UINT primCount);

	void DeviceLoadVertexDataPOSITIONTVerticesPosition(const D3DPRIMITIVETYPE PrimitiveType, const UINT primCount, const bool useIndices, const INT BaseVertexIndex, const UINT startIndex);
	void DeviceLoadVertexDataPOSITIONTVerticesAttributes(const D3DPRIMITIVETYPE PrimitiveType, const UINT primCount, const bool useIndices, const INT BaseVertexIndex, const UINT startIndex);

	void DeviceSetCurrentState(const D3DPRIMITIVETYPE primType, const unsigned char indexSize);
	...
	void CallRunCommandList(const GPUCommandList& runCommandList);
	...
	IBaseDeviceComms* const GetDeviceComms() const
	{
		return deviceComms;
	}

	IBaseGPUDevice* const GetBaseDevice() const
	{
		return baseDevice;
	}

	const bool GetEnableSoftwareRenderingVisualization() const
	{
		return enableSoftwareRenderingVisualization;
	}

	void SetEnableSoftwareRenderingVisualization(const bool newSoftwareRenderingEnable)
	{
		enableSoftwareRenderingVisualization = newSoftwareRenderingEnable;
	}

	GPUStats& GetDeviceStats()
	{
		return deviceStats;
	}
	...
	{
		return enableScanout;
	}

	void SetScanoutEnabled(const bool newEnableScanout)
	{
		enableScanout = newEnableScanout;
	}

	const bool GetUseDoubleBuffering() const
	{
		return useDoubleBuffering;
	}

	void SetUseDoubleBuffering(const bool newUseDoubleBuffering)
	{
	...
	void SetEmulateCommandLists(const bool bNewEmulateCommandLists)
	{
...
// Swizzles from BGRA -> RGBA keeping everything as a DWORD (not converting to float and then converting back)
inline const RGBACOLOR D3DColorToRGBAColor(const D3DCOLOR inColor)
{
	const unsigned char r = (const unsigned char)( (inColor >> 16) & 0xFF);
	const unsigned char g = (const unsigned char)( (inColor >> 8) & 0xFF);
	const unsigned char b = (const unsigned char)(inColor & 0xFF);
	const unsigned char a = (const unsigned char)( (inColor >> 24) & 0xFF);

	return ( (const RGBACOLOR)r) | 
		( ( (const RGBACOLOR)g) << 8) | 
		( ( (const RGBACOLOR)b) << 16) | 
		( ( (const RGBACOLOR)a) << 24);
}
...
inline const RGB565 Narrow888To565(const D3DCOLOR inColor8)
{
	const unsigned char r8 = (const unsigned char)( (inColor8 >> 16) & 0xFF);
	const unsigned char g8 = (const unsigned char)( (inColor8 >> 8) & 0xFF);
	const unsigned char b8 = (const unsigned char)(inColor8 & 0xFF);
	const unsigned char a8 = (const unsigned char)( (inColor8 >> 24) & 0xFF);
	RGB565 ret;
	ret.bits565.r = r8 >> 3;
	ret.bits565.g = g8 >> 2;
	ret.bits565.b = b8 >> 3;
...
inline const A4R4G4B4 Narrow8888To4444(const D3DCOLOR inColor8)
{
	const unsigned char r8 = (const unsigned char)( (inColor8 >> 16) & 0xFF);
	const unsigned char g8 = (const unsigned char)( (inColor8 >> 8) & 0xFF);
	const unsigned char b8 = (const unsigned char)(inColor8 & 0xFF);
...
inline const X4R4G4B4 Narrow8888To4440(const D3DCOLOR inColor8)
{
	const unsigned char r8 = (const unsigned char)( (inColor8 >> 16) & 0xFF);
	const unsigned char g8 = (const unsigned char)( (inColor8 >> 8) & 0xFF);
	const unsigned char b8 = (const unsigned char)(inColor8 & 0xFF);
