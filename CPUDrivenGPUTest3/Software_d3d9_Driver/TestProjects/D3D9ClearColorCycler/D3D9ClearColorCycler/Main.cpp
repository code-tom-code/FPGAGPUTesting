...
static const inline float saturateF(const float f)
{
	if (f > 1.0f)
		return 1.0f;
	else if (f < 0.0f)
...
static inline void RenderFrame(const long double timeDelta)
{
	...
	const float r = saturateF(thisFrameColor.x) * 255.0f;
	const float g = saturateF(thisFrameColor.y) * 255.0f;
	const float b = saturateF(thisFrameColor.z) * 255.0f;
	const unsigned ur = (const unsigned)r;
	const unsigned ug = (const unsigned)g;
	const unsigned ub = (const unsigned)b;

	d3d9dev->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(ur, ug, ub), 1.0f, 0x00000000);
	d3d9dev->BeginScene();
