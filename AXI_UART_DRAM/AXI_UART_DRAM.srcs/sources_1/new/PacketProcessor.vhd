...
entity PacketProcessor is
	...
	port (
		...
		FIFO_OUT_VALID_PACKETS_DIN : out STD_LOGIC_VECTOR(PACKET_SIZE_IN_BYTES*8-1 downto 0);
		FIFO_OUT_VALID_PACKETS_WR_EN : out std_logic;

		FIFO_IN_RETURN_PACKETS_EMPTY : in std_logic;
		FIFO_IN_RETURN_PACKETS_DOUT : in STD_LOGIC_VECTOR(PACKET_SIZE_IN_BYTES*8-1 downto 0);
		FIFO_IN_RETURN_PACKETS_RD_EN : out std_logic;

		-- Debug signals
		DBG_PACKETSTATE : out STD_LOGIC_VECTOR(7 downto 0);
		DBG_INCPACKETBUFFER : out STD_LOGIC_VECTOR(PACKET_SIZE_IN_BYTES*8-1 downto 0)
		);
end PacketProcessor;
...
begin
	...
	-- Output validated packets FIFO signals:
	FIFO_OUT_VALID_PACKETS_DIN <= validated_packet_data;
					...
					when PACKETPROC_CHECK_RETURN_QUEUE =>
						if (FIFO_IN_RETURN_PACKETS_EMPTY = '0') then
							...
							for byte in 0 to PACKET_SIZE_IN_BYTES-1 loop
								partialPacketBuffer(byte) <= FIFO_IN_RETURN_PACKETS_DOUT( ( (byte + 1) * 8 - 1) downto (byte * 8) );
							end loop;
