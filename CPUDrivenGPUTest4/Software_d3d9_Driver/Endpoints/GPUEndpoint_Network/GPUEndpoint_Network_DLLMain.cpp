...
static INetSocketDeviceComms* networkDevice = NULL;
...
bool __stdcall InitEndpointImpl(const ReturnMessageSignature D2HReplyCallback)
{
	...
	networkDevice = new INetSocketDeviceComms();
...
void __stdcall ProcessNewMessageImpl(const genericCommand* H2DCommandPacket)
{
	...
	networkDevice->SendLoop(reinterpret_cast<const BYTE* const>(H2DCommandPacket), sizeof(genericCommand) );
}
