...
void IDirect3DSurface9Hook::UpdateSurfaceToGPU()
{
#ifdef _DEBUG
	...
	if (InternalWidth > 128 || InternalHeight > 128) // In the future, this limitation can be changed from "dimension > 128" to "uncompressed texture fits in 64KB" (need to code HDL for supporting textures > 128 for that)
	{
		...
		unsigned xShift = 0;
		unsigned yShift = 0;
		const unsigned reducedDimensions = GetReducedTextureDimensions(xShift, yShift);
		...
		{
			const float yTexcoord = y * yTexScale;
			for (unsigned x = 0; x < resizeWidth; ++x)
			{
				D3DXVECTOR4 filteredColor;
				SampleSurfaceInternal<0xF, false>(x * xTexScale, yTexcoord, D3DTEXF_LINEAR, filteredColor);
				const D3DCOLOR color8 = Float4ToD3DCOLOR(filteredColor);
				switch (InternalFormat)
				{
				case D3DFMT_A8R8G8B8:
				case D3DFMT_X8R8G8B8:
					( (D3DCOLOR* const)resizeBuffer)[x + y * resizeWidth] = color8;
					break;
				case D3DFMT_R5G6B5:
					( (RGB565* const)resizeBuffer)[x + y * resizeWidth] = Narrow888To565(color8);
					break;
				case D3DFMT_A4R4G4B4:
					( (A4R4G4B4* const)resizeBuffer)[x + y * resizeWidth] = Narrow8888To4444(color8);
					break;
				case D3DFMT_X4R4G4B4:
					( (X4R4G4B4* const)resizeBuffer)[x + y * resizeWidth] = Narrow8888To4440(color8);
					break;
				case D3DFMT_DXT1:
				case D3DFMT_DXT3:
				case D3DFMT_DXT5:
				...
				default:
				{
					__debugbreak(); // Unsupported format for resizing!
				}
				}
			}
		}
	...
	const DWORD endTick = GetTickCount();

	printf("...finished copying texture in %ums\n", endTick - startTick);
}

const UINT IDirect3DSurface9Hook::GetReducedTextureDimensions(unsigned& outXShift, unsigned& outYShift) const
{
#ifdef _DEBUG
	if (InternalWidth == 0 || InternalHeight == 0 || InternalWidth > 0xFFFF || InternalHeight > 0xFFFF)
	{
		__debugbreak();
	}
#endif
...
#ifdef _DEBUG
	if (__popcnt(InternalWidth) > 1 || __popcnt(InternalHeight) > 1)
	{
		// This only works on power of 2 textures (Width and Height do not need to match)
		__debugbreak();
	}
#endif

	unsigned resizeWidth = InternalWidth;
	unsigned xShift = 0;
	...
	return (resizeWidth) | ( (resizeHeight) << 16);
}
...
#ifdef _DEBUG
void PrintDebugSurfaceName(char* const outString, UINT _Width, UINT _Height, D3DFORMAT _Format, const char* const CreationType)
{
#pragma warning(push)
#pragma warning(disable:4996)
	sprintf(outString, "Surface [%s] (W:%u, H:%u, Fmt:%s)", CreationType, _Width, _Height, GetAbbreviatedFormatStringFromFormat(_Format) );
#pragma warning(pop)
}
#endif // #ifdef _DEBUG
...
void IDirect3DSurface9Hook::CreateOffscreenPlainSurface(UINT _Width, UINT _Height, D3DFORMAT _Format, D3DPOOL _Pool)
{
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, _Width, _Height, _Format, "OffscreenPlain");
#endif
	...
	{
		GPUSurfaceBytesRaw = GPUAlloc(GetSurfaceSizeBytes(InternalWidth, InternalHeight, InternalFormat), GPUVAT_RenderTarget, ConvertD3DFormatToDeviceFormat(InternalFormat)
#ifdef _DEBUG
			, debugObjectName
#endif
		);
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, _Width, _Height, _Format, "DepthStencil");
#endif

	GPUSurfaceBytesRaw = GPUAlloc(GetSurfaceSizeBytes(InternalWidth, InternalHeight, InternalFormat), GPUVAT_DepthStencil, ConvertD3DFormatToDeviceFormat(InternalFormat)
#ifdef _DEBUG
			, debugObjectName
#endif
		);
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, _Width, _Height, _Format, "RenderTarget");
#endif
...
void IDirect3DSurface9Hook::CreateTextureImplicitSurface(UINT _Width, UINT _Height, D3DFORMAT _Format, D3DPOOL _Pool, const DebuggableUsage _Usage, UINT _Level, IDirect3DTexture9Hook* _HookParentTexturePtr)
{
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, _Width, _Height, _Format, "TextureImplicit");
#endif
	...
	if (IsCompressedFormat(InternalFormat) )
	{
		...
		{
			GPUSurfaceBytesRaw = GPUAlloc(GetSurfaceSizeBytes( (InternalWidth + 3) & ~3, (InternalHeight + 3) & ~3, D3DFMT_A8R8G8B8), GPUVAT_Texture, ConvertD3DFormatToDeviceFormat(D3DFMT_A8R8G8B8)
#ifdef _DEBUG
				, debugObjectName
#endif
			);
	...
	else
	{
		if (TextureSurfaceLevel == 0 && _Pool <= D3DPOOL_MANAGED)
		{
			GPUSurfaceBytesRaw = GPUAlloc(GetSurfaceSizeBytes(InternalWidth, InternalHeight, InternalFormat), GPUVAT_Texture, ConvertD3DFormatToDeviceFormat(InternalFormat)
#ifdef _DEBUG
				, debugObjectName
#endif
			);
...
void IDirect3DSurface9Hook::CreateDeviceImplicitSurface(const D3DPRESENT_PARAMETERS& d3dpp)
{
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, InternalWidth, InternalHeight, InternalFormat, "DeviceImplicitRT");
#endif
	...
	else
	{
		...
		GPUSurfaceBytesRaw = GPUAllocAtAddress( (gpuvoid* const)0x20000000, GetSurfaceSizeBytes(InternalWidth, InternalHeight, InternalFormat), GPUVAT_RenderTarget, ConvertD3DFormatToDeviceFormat(InternalFormat)
#ifdef _DEBUG
			, debugObjectName
#endif
		);
...
void IDirect3DSurface9Hook::CreateDeviceImplicitFrontbufferSurface(const D3DPRESENT_PARAMETERS& d3dpp)
{
	creationMethod = deviceImplicitFrontbuffer;
	InternalWidth = d3dpp.BackBufferWidth;
	InternalHeight = d3dpp.BackBufferHeight;
	InternalFormat = d3dpp.BackBufferFormat;
	InternalPool = D3DPOOL_DEFAULT;
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, InternalWidth, InternalHeight, InternalFormat, "DeviceImplicitFrontbuffer");
#endif
	...
	{
		// Alloc the initial implicit device backbuffer at an exact device address:
		GPUSurfaceBytesRaw = GPUAllocAtAddress( (gpuvoid* const)0x30000000, GetSurfaceSizeBytes(InternalWidth, InternalHeight, InternalFormat), GPUVAT_RenderTarget, ConvertD3DFormatToDeviceFormat(InternalFormat)
#ifdef _DEBUG
			, debugObjectName
#endif
		);
...
void IDirect3DSurface9Hook::CreateDeviceImplicitDepthStencil(const D3DPRESENT_PARAMETERS& d3dpp)
{
...
#ifdef _DEBUG
	PrintDebugSurfaceName(debugObjectName, InternalWidth, InternalHeight, InternalFormat, "DeviceImplicitDS");
#endif
	...
	GPUSurfaceBytesRaw = GPUAllocAtAddress( (gpuvoid* const)0x10000000, GetSurfaceSizeBytes(InternalWidth, InternalHeight, InternalFormat), GPUVAT_DepthStencil, ConvertD3DFormatToDeviceFormat(InternalFormat)
#ifdef _DEBUG
			, debugObjectName
#endif
		);
