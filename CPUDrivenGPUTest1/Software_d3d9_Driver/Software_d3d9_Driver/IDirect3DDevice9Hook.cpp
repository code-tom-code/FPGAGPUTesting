...
COM_DECLSPEC_NOTHROW HRESULT STDMETHODCALLTYPE IDirect3DDevice9Hook::DrawPrimitive(THIS_ D3DPRIMITIVETYPE PrimitiveType, UINT StartVertex, UINT PrimitiveCount)
{
	...
	if (SkipVertexProcessing() )
	{
...

		{
			...
			DeviceSetCurrentState(PrimitiveType, 0); // Update the device state
	...
	DeviceLoadVertexDataProcessedVerticesPosition(PrimitiveType, PrimitiveCount); // Upload the vertex data to the GPU

	{
		GPUCommandList newRecordingCommandList;
		baseDevice->BeginRecordingCommandList(&newRecordingCommandList);
		DeviceLoadVertexDataProcessedVerticesAttributes(PrimitiveType, PrimitiveCount); // Upload the vertex data to the GPU
		DeviceSetCurrentState(PrimitiveType, 0); // Update the device state
		baseDevice->DeviceDrawPrimitive(PrimitiveType, PrimitiveCount);
...
#endif
	, const bool bIsIndexBuffer /*= false*/
)
{
	...
	newBuffer.deviceMemory = GPUAlloc(newBuffer.deviceSizeBytes, bIsIndexBuffer ? GPUVAT_IndexData : GPUVAT_VertexStreamData, newBuffer.format 
#ifdef _DEBUG
		, debugAllocString
#endif
	);
...
void IDirect3DDevice9Hook::DeviceLoadIndexBuffer(const unsigned short* const indices, const unsigned numIndices)
{
#ifdef _DEBUG
	if (!indices)
	{
...
#endif
#ifdef PRINT_COMMS
	printf("Uploading %u indices...\n", numIndices);
#endif
	deviceAllocatedBuffer indicesAlloc;
	indicesAlloc.deviceSizeBytes = numIndices * sizeof(USHORT);
	indicesAlloc.format = GPUFMT_Index_UINT16;
	indicesAlloc.bufferHash = deviceAllocatedBuffer::ComputeHash(indices, numIndices * sizeof(USHORT) );
	const bool bIsIndexData = true;
	if (CreateOrUseCachedVertDataBuffer(indicesAlloc, cachedIndexBuffers
#ifdef _DEBUG
		, "IndexBuffer_UINT16"
#endif
		, bIsIndexData
	...
	{
		deviceComms->DeviceMemCopy(indicesAlloc.deviceMemory, indices, numIndices * sizeof(USHORT) );
	}
	baseDevice->DeviceSetIndexBuffer(indicesAlloc.deviceMemory, numIndices);
}
...
void IDirect3DDevice9Hook::DeviceSetCurrentState(const D3DPRIMITIVETYPE primType, const unsigned char indexSize)
{
	...
	// Set the IA state:
	baseDevice->DeviceSetIAState(cullMode, primTopology, sct_CutDisabled, indexFormat);
...
COM_DECLSPEC_NOTHROW HRESULT STDMETHODCALLTYPE IDirect3DDevice9Hook::DrawIndexedPrimitive(THIS_ D3DPRIMITIVETYPE PrimitiveType, INT BaseVertexIndex, UINT MinVertexIndex, UINT NumVertices, UINT startIndex, UINT primCount)
{
	...
	if (SkipVertexProcessing() )
	{
...

		{
			...
			DeviceSetCurrentState(PrimitiveType, currentState.currentIndexBuffer->GetFormat() == D3DFMT_INDEX16 ? 16 : 32); // Update the device state
	...
	DeviceLoadVertexDataProcessedVerticesPosition(PrimitiveType, primCount); // Upload the vertex data to the GPU

	{
		...
		DeviceLoadIndexBuffer( ( (const USHORT* const)currentState.currentIndexBuffer->GetBufferBytes() ), currentState.currentIndexBuffer->GetInternalLength() / sizeof(USHORT) );
		DeviceLoadVertexDataProcessedVerticesAttributes(PrimitiveType, primCount); // Upload the vertex data to the GPU
		DeviceSetCurrentState(PrimitiveType, currentState.currentIndexBuffer->GetFormat() == D3DFMT_INDEX16 ? 16 : 32); // Update the device state
