...
const unsigned FloatToUNORM(float fVal)
{
	static_assert(numBits > 0, "Error: Function doesn't support 0 bit values");
	static_assert(numBits <= 24, "Error: This imprecise function doesn't accurately handle 32-bit or higher floats yet");
	if (isnan(fVal) )
		return 0;
	if (fVal > 1.0f)
		fVal = 1.0f;
		...
		fVal = 0.0f;
	const unsigned baseScale = (1 << numBits) - 1;
	fVal = fVal * baseScale + 0.5f;
...
#pragma warning(push)
#pragma warning(disable:4996)
		FILE* const fWrite = fopen(outputFilename, "wb");
#pragma warning(pop)
		if (!fWrite)
		{
			__debugbreak();
		...
		{
			screencapBuffer[x] = (const D3DCOLOR)D3DColorToRGBAColor(screencapBuffer[x]); // Swizzle ARGB -> ABGR
		}

		tgaHeader outHeader = {0};
		outHeader.width = 640;
		outHeader.height = 480;
		outHeader.bitsperpixel = 32; // 32 bpp
		outHeader.colormaptype = 0; // Colormap 0
		outHeader.datatypecode = 2; // Type 2 image (uncompressed RGBA)
		outHeader.colordepth = 32; // 32 bit color depth
		outHeader.imageDescriptor[0] = 1 << 5; // Use top-left origin for this image
		fwrite(&outHeader, sizeof(outHeader), 1, fWrite);
		fwrite(screencapBuffer, sizeof(screencapBuffer), 1, fWrite);
		fclose(fWrite);
		printf("Screencap dump completed after %ums!\n", endTicks - startTicks);
...

		{
			...
			//if (currentTime - lastPrintTime > 33)
			{
				...
				if (initialCreateFocusWindow)
					mainWindowWnd = initialCreateFocusWindow;
				else
...
void IDirect3DDevice9Hook::CallRunCommandList(const GPUCommandList& runCommandList)
{
	if (emulateCommandLists)
		baseDevice->DeviceEmulateExecuteCommandList(runCommandList);
	else
...
)
{
	...
	{
#ifdef _DEBUG
		__debugbreak(); // Do not call this function without first calling ComputeHash() on the deviceAllocatedBuffer!
#endif
		return true;
	}
	...
	if (newBuffer.format >= GPUFMT_StartValidVertexFormats && newBuffer.format <= GPUFMT_StartValidVertexFormats)
	{
		deviceAllocationUsage = GPUVAT_VertexStreamData;
	}
	else if (newBuffer.format >= GPUFMT_StartValidIndexFormats && newBuffer.format <= GPUFMT_EndValidIndexFormats)
	{
		deviceAllocationUsage = GPUVAT_IndexData;
	}
	else if (newBuffer.format >= GPUFMT_StartValidConstBufferFormats && newBuffer.format <= GPUFMT_EndValidConstBufferFormats)
	{
		deviceAllocationUsage = GPUVAT_ConstantBufferData;
	}
	else
	{
#ifdef _DEBUG
...
const bool AllVectorElementsEqual(const std::vector<T>& testVector)
{
	...
	const unsigned numElements = testVector.size();
	for (unsigned x = 1; x < numElements; ++x)
	{
		if (!(testVector[x] == testVector[x - 1]) )
			return false;
	}
...
void IDirect3DDevice9Hook::DeviceSetVertexStreamsAndDecl()
{
	...
	{
#ifdef _DEBUG
		__debugbreak(); // No vertex decl currently set? Then we can't draw anything!
#endif
		return;
	}
	...
	{
#ifdef _DEBUG
		__debugbreak(); // No vertex shader currently set? Then we can't draw anything!
#endif
		return;
	}
	...
	for (unsigned elementIndex = 0; elementIndex < totalNumElements; ++elementIndex)
	{
		...
		case D3DDECLTYPE_FLOAT1:
			elementDWORDCount = sizeof(FLOAT) / sizeof(DWORD);
			break;
		...
		case D3DDECLTYPE_FLOAT2:
			elementDWORDCount = sizeof(FLOAT) * 2 / sizeof(DWORD);
			break;
		case D3DDECLTYPE_FLOAT3:
			elementDWORDCount = sizeof(FLOAT) * 3 / sizeof(DWORD);
			break;
		case D3DDECLTYPE_FLOAT4:
			elementDWORDCount = sizeof(FLOAT) * 4 / sizeof(DWORD);
			break;
		default:
#ifdef _DEBUG
			__debugbreak(); // Unknown element type!
#endif
			elementDWORDCount = 0;
		}
		...
		// It is legal to have more vertex stream elements that are not consumed by the shader, so we need to check here to see if the shader expects to use them or not:
		if (shaderRegIndexFound)
		{
			if (shaderRegIndex >= GPU_SHADER_MAX_NUM_INPUT_REG)
			{
#ifdef _DEBUG
				__debugbreak(); // Shader register index is out of bounds for the hardware!
#endif
				return;
			}
...
// The format of the viewport transform VS constant register is currently expected to be:
...
const float4 IDirect3DDevice9Hook::GetDeviceViewportConstantF() const
{
	float4 retRegister;

	retRegister.x = currentState.cachedViewport.halfWidthF;
	retRegister.y = currentState.cachedViewport.halfHeightF;
	retRegister.z = currentState.cachedViewport.zScale;
	retRegister.w = currentState.cachedViewport.viewport.MinZ;

	return retRegister;
}

void IDirect3DDevice9Hook::DeviceSetUsedVertexShaderConstants()
{
	if (!currentState.currentVertexShader)
	{
#ifdef _DEBUG
	...
	struct constantRegisterRange
	{
		unsigned char rangeStartIndex;
		unsigned char rangeRegisterLength;
	};

	const DeviceBytecode* const deviceShaderInfo = currentState.currentVertexShader->GetDeviceCompiledShaderInfo();
	if (!deviceShaderInfo)
	{
#ifdef _DEBUG
		__debugbreak(); // We have no device vertex shader? Did we fail shader compilation or GPU allocation?
#endif
		return;
	}
	...
	if (numUsedConstFRegisters > 0)
	{
		...
		if (IsCurrentlyRecordingStateBlock() )
			targetDeviceState = currentlyRecordingStateBlock->GetDeviceStateForRead();
		else
		...
		for (unsigned x = 0; x < GPU_SHADER_MAX_NUM_CONSTANT_FLOAT_REG;)
		{
			if (usedConstRegistersBitmap[x / 32])
			{
				if (usedConstRegistersBitmap[x / 32] & (1 << (x % 32) ) )
				{
					if (!isInRange)
					{
						constantRegisterRange newRange;
						newRange.rangeStartIndex = x;
						newRange.rangeRegisterLength = 1;
						registerRanges.push_back(newRange);
					}
					else
					{
						constantRegisterRange& currentRange = registerRanges.back();
						++currentRange.rangeRegisterLength;
					}

					const float4& thisConstRegisterData = targetDeviceState->vertexShaderRegisters.floats[x];
					constRegisterData.push_back(thisConstRegisterData);
		...
		const unsigned numRegisterRanges = registerRanges.size();
		unsigned numPaddingFloat4s = 0;
		for (unsigned registerRangeIndex = 0; registerRangeIndex < numRegisterRanges - 1; ++registerRangeIndex)
		{
			// For each odd-length register range (except for the last range), add one padding register so that the start of each range
			// falls on a 32-byte boundary!
			numPaddingFloat4s += (registerRanges[registerRangeIndex].rangeRegisterLength & 0x1);
		}

	#ifdef _DEBUG
		if (constRegisterData.size() + numPaddingFloat4s >= GPU_SHADER_MAX_NUM_CONSTANT_FLOAT_REG)
		{
			__debugbreak();
		}
	#endif
	...
	{
		const InitialConstantValue& thisInitialConst = vsShaderInfo.initialConstantValues[x];
#ifdef _DEBUG
		if (thisInitialConst.constantRegisterIndex >= GPU_SHADER_MAX_NUM_CONSTANT_FLOAT_REG)
		{
			__debugbreak();
		}
#endif
...
void IDirect3DDevice9Hook::DeviceSetCurrentState(const D3DPRIMITIVETYPE primType, const IDirect3DIndexBuffer9Hook* currentIB)
{
	...
	{
		if (currentState.currentStageStates[0].stageStateUnion.namedStates.colorArg1 == TA_DIFFUSE || currentState.currentStageStates[0].stageStateUnion.namedStates.colorArg1 == TA_CURRENT)
		{
			cbModeColor = cbm_vertexColorOnly;
		}
		else if (currentState.currentStageStates[0].stageStateUnion.namedStates.colorArg1 == TA_TEXTURE)
		{
			cbModeColor = cbm_textureOnly;
		}
		else
		{
			cbModeColor = cbm_allWhite;
		}
	}
	else if (currentState.currentStageStates[0].stageStateUnion.namedStates.colorOp == D3DTOP_SELECTARG2)
	{
		if (currentState.currentStageStates[0].stageStateUnion.namedStates.colorArg2 == TA_DIFFUSE || currentState.currentStageStates[0].stageStateUnion.namedStates.colorArg2 == TA_CURRENT)
		{
			cbModeColor = cbm_vertexColorOnly;
		}
		else if (currentState.currentStageStates[0].stageStateUnion.namedStates.colorArg2 == TA_TEXTURE)
		{
			cbModeColor = cbm_textureOnly;
		}
		else
	...
	else // if (currentState.currentStageStates[0].stageStateUnion.namedStates.colorOp == D3DTOP_MODULATE)
	{
		cbModeColor = cbm_textureModulateVertexColor;
	}
	if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaOp == D3DTOP_SELECTARG1)
	{
		if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaArg1 == TA_DIFFUSE || currentState.currentStageStates[0].stageStateUnion.namedStates.alphaArg1 == TA_CURRENT)
		{
			cbModeAlpha = cbm_vertexColorOnly;
		}
		else if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaArg1 == TA_TEXTURE)
		{
			cbModeAlpha = cbm_textureOnly;
		}
		else
		{
	...
	else if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaOp == D3DTOP_SELECTARG2)
	{
		if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaArg2 == TA_DIFFUSE || currentState.currentStageStates[0].stageStateUnion.namedStates.alphaArg2 == TA_CURRENT)
		{
			cbModeAlpha = cbm_vertexColorOnly;
		}
		else if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaArg2 == TA_TEXTURE)
		{
			cbModeAlpha = cbm_textureOnly;
		}
		else
	...
	else // if (currentState.currentStageStates[0].stageStateUnion.namedStates.alphaOp == D3DTOP_MODULATE)
	{
	...
	if (currentTex0)
	{
		IDirect3DSurface9Hook* surface0hook = NULL;
		currentTex0->GetSurfaceLevel(0, (IDirect3DSurface9** const)&surface0hook);
...
void IDirect3DDevice9Hook::DrawPrimitiveUB(const D3DPRIMITIVETYPE PrimitiveType, const UINT PrimitiveCount) const
{
	...
	// Early-out if there is no work to be done by the driver!
	if (!enableSoftwareRenderingVisualization)
		return;
