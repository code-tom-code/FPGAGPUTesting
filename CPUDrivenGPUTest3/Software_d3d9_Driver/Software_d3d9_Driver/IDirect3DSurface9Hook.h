...
public:
	...
	void CreateDeviceImplicitFrontbufferSurface(const D3DPRESENT_PARAMETERS& d3dpp);
	...
	gpuvoid* GetDeviceSurfaceBytes(void) const
	{
	...
	const UINT GetReducedTextureDimensions(unsigned& outXShift, unsigned& outYShift) const;
	...
	void UpdateSurfaceToGPU();

	void DumpSurfaceToDisk(void) const;
	static void InitDumpSurfaces(void);

	void FrontbufferBackbufferSwap(IDirect3DSurface9Hook* const backbufferPtr);
	...
	void UpdateSurfaceToGPUIfDirty();
